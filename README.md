...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create web-7bdc
  fly apps create web-7bdc-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app web-7bdc
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app web-7bdc-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'web-7bdc-staging' does not match app name in config file 'web-7bdc'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name web-7bdc-db
  fly postgres attach --app web-7bdc web-7bdc-db

  fly postgres create --name web-7bdc-staging-db
  fly postgres attach --app web-7bdc-staging web-7bdc-staging-db
  ```
